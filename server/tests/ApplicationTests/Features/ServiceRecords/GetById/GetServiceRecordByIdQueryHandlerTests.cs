...
namespace ApplicationTests.Features.ServiceRecords.GetById;
...
public class GetServiceRecordByIdQueryHandlerTests : InMemoryDbTestBase
{
    ...
    public async Task Handle_VehicleNotOwnedByUser_ReturnsNotFoundError()
    {
        ...
        var serviceRecord = await CreateServiceRecordInDb(vehicle.Id, ServiceRecordType.OilAndFilters, "Oil Change", DateTime.UtcNow.AddDays(-5), 10000, 150.00m);
    ...
    public async Task Handle_ServiceRecordBelongsToDifferentVehicle_ReturnsNotFoundError()
    {
        ...
        var serviceRecord = await CreateServiceRecordInDb(vehicle1.Id, ServiceRecordType.OilAndFilters, "Oil Change", DateTime.UtcNow.AddDays(-5), 10000, 150.00m);
    ...
    public async Task Handle_ValidServiceRecordWithoutItems_ReturnsServiceRecordDto()
    {
        ...
        result.Value.Type.ShouldBe(ServiceRecordType.OilAndFilters);
    ...
    public async Task Handle_ValidServiceRecordWithItems_ReturnsServiceRecordDtoWithItems()
    {
        ...
        // Add service items
        var item1 = await CreateServiceItemInDb(serviceRecord.Id, "Engine Oil 5W-30", ServiceItemType.Part, 45.00m, 5, "OIL-123", "Premium synthetic");
        var item2 = await CreateServiceItemInDb(serviceRecord.Id, "Oil Filter", ServiceItemType.Part, 15.00m, 1, "FILTER-456");
        var item3 = await CreateServiceItemInDb(serviceRecord.Id, "Labor", ServiceItemType.Labor, 80.00m, 1, null, "Oil change service");
