...
public class CreateFuelEntryCommandHandlerTests : InMemoryDbTestBase
{
    ...
    public async Task Handle_ValidCommand_ReturnsSuccessWithFuelEntryDto()
    {
        ...
        _vehicleEnergyValidatorMock.Setup(x => x.CanBeCharged(PowerType.Gasoline)).Returns(false);
    ...
    public async Task PowerType_CanBeCharged_ReturnsIncompatiblePowerTypeError(PowerType powerType)
    {
        // Arrange
        SetupAuthorizedUser();
    ...
    public async Task PowerType_CannotBeCharged_ReturnsSuccess(PowerType powerType)
    {
        // Arrange
        SetupAuthorizedUser();
        var vehicle = await CreateVehicleInDb(powerType);
